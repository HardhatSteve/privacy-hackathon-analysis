...
// =============================================================================
// MERKLE UPDATE IN-PLACE
// =============================================================================
// Update leaf at pathIndex, return both old and new roots using same siblings
template MerkleUpdateInPlace(HEIGHT) {
...
// =============================================================================
// MERKLE APPEND WITH PROOF
// =============================================================================
// Append leaf at index using full merkle proof (not subtree caching).
...
template MerkleAppendWithProof(HEIGHT) {
    signal input leaf;
    signal input index;
    ...
    // Precomputed zero hashes for each level (Poseidon)
    ...
    // IMPORTANT: Uses 3-input Poseidon for indexed leaf hash, NOT 2-input
    var ZERO_HASHES[26];
    ...
    old_hashes[0] <== ZERO_HASHES[0];  // Empty leaf = zero hash at level 0
...
// =============================================================================
// SINGLE NULLIFIER INSERTION
// =============================================================================
// Proves one insertion: update low element + append new leaf
...
template SingleNullifierInsert(HEIGHT) {
    // Inputs
    ...
    component update_low = MerkleUpdateInPlace(HEIGHT);
    ...
    // Intermediate root after low element update (NOW USED!)
    ...
    component append = MerkleAppendWithProof(HEIGHT);
...
// =============================================================================
// NULLIFIER BATCH INSERT (SIMPLE - ALL IN_TREE)
// =============================================================================
...
template NullifierBatchInsertSimple(HEIGHT, BATCH_SIZE) {
    ...
    signal input append_merkle_proofs[BATCH_SIZE][HEIGHT];  // NEW: proofs against intermediate roots
    ...
    for (var i = 0; i < BATCH_SIZE; i++) {
        inserts[i] = SingleNullifierInsert(HEIGHT);
