        ...
        <table className="w-full table-fixed text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-50 text-xs sm:text-sm">
            {records.map((r) => (
              <tr
                key={r.txId}
                className="group transition-all duration-200 hover:bg-linear-to-r hover:from-indigo-50/50 hover:to-transparent hover:shadow-sm"
              >
                {/* 文件名 */}
                <td
                  className="truncate px-4 py-4 font-bold text-slate-900 sm:px-6"
                  title={r.fileName}
                >
                ...
                {/* 文件类型 */}
                <td
                  className="hidden truncate px-4 py-4 text-slate-600 sm:px-6 lg:table-cell"
                  title={r.mimeType || "-"}
                >
                ...
                {/* 文件大小 */}
                <td
                  className="hidden truncate px-4 py-4 text-slate-600 sm:px-6 xl:table-cell"
                ...
                <td className="hidden truncate px-4 py-4 sm:px-6 xl:table-cell">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="group/tx block truncate font-mono text-xs text-indigo-600 hover:text-indigo-700 hover:underline"
                    title={r.txId}
                  >
                ...
                <td className="truncate px-4 py-4 sm:px-6">
                  {r.encryptionAlgo !== "none" ? (
                    ...
                    <span
                      className="block truncate text-[10px] font-bold text-slate-400 uppercase"
                ...
                <td className="px-4 py-4 text-right sm:px-6">
                  <div className="flex items-center justify-end gap-1 sm:gap-2">
                    <Button
                      ...
                      size="icon"
                      asChild
                      className="h-8 w-8 text-slate-400 transition-all duration-200 hover:scale-110 hover:bg-indigo-50 hover:text-indigo-600 active:scale-95 sm:h-9 sm:w-9"
                    >
                      <a
                        href={`https://arweave.net/${r.txId}`}
                        target="_blank"
