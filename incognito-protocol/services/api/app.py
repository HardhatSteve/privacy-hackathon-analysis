    ...
    try:
        ...
        async with get_async_session() as session:
            ...
            if existing_note.scalar_one_or_none():
                print(f"ℹ️  Note with commitment {commitment[:8]}... already exists in database, skipping")
                return
...
async def _mark_note_spent_db(commitment: str, nullifier: str, owner_pub: str = None) -> None:
    ...
    try:
        ...
        print(f"❌ Database mark spent failed: {e}")
        raise
...
async def withdraw(req: WithdrawReq):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if len(synced_leaves) == 0:
                print(f"[WITHDRAW] ⚠️  On-chain history empty, using local pool_merkle_state.json")
                pool_merkle_path = Path(REPO_ROOT) / "pool_merkle_state.json"
                if pool_merkle_path.exists():
                    with open(pool_merkle_path, 'r') as f:
                        local_state = json.load(f)
            ...
            print(f"[WITHDRAW] ✅ Local tree synced with {len(synced_leaves)} leaves")

    except Exception as e:
        print(f"[WITHDRAW] ⚠️  Could not sync with on-chain, using local file: {e}")
    ...
    if leaf_index >= len(tree.leaves):
        ...
        print(f"[WITHDRAW] ⚠️  Claimed leaf_index {leaf_index} out of bounds, searching tree...")
    ...
    if expected_leaf != actual_leaf:
        ...
        print(f"[WITHDRAW] ⚠️  Leaf mismatch at index {leaf_index}, searching tree...")
        for idx, leaf in enumerate(tree.leaves):
            if leaf == expected_leaf:
...
async def marketplace_buy(req: BuyReq):
    ...
    if has_note_credentials:
        ...
        print(f"[PURCHASE] ✅ Buyer's note marked as spent")
    ...
    if has_note_credentials and withdrawable_amount > total_price:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                try:
                    ...
                    if len(existing_leaves) == 0:
                        print(f"[PURCHASE] ⚠️  On-chain history empty, using local pool_merkle_state.json")
                        pool_merkle_path = Path(REPO_ROOT) / "pool_merkle_state.json"
                        if pool_merkle_path.exists():
                            with open(pool_merkle_path, 'r') as f:
                                local_state = json.load(f)
                    ...
                    print(f"[PURCHASE] ❌ JSON parse error: {json_err}")
                ...
                print(f"[PURCHASE] ❌ get_deposit_history failed:")
            ...
            print(f"[PURCHASE] ❌ Error fetching on-chain state: {e}")
            raise
        ...
        try:
            ...
            print(f"[PURCHASE] ✅ Change note added to on-chain tree at index {change_leaf_index}")
        ...
        print(f"[PURCHASE] ✅ Change note created, synced, and saved in DATABASE for buyer")
...
async def escrow_claim(
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            try:
                ...
                # Fallback to local file if on-chain history is empty (e.g., localnet restarted)
                if len(existing_leaves) == 0:
                    print(f"[ESCROW_CLAIM] ⚠️  On-chain history empty, using local pool_merkle_state.json")
                    pool_merkle_path = Path(REPO_ROOT) / "pool_merkle_state.json"
                    if pool_merkle_path.exists():
                        with open(pool_merkle_path, 'r') as f:
                            local_state = json.load(f)
                ...
                print(f"[ESCROW_CLAIM] ❌ JSON parse error: {json_err}")
                print(f"[ESCROW_CLAIM] stdout: {result.stdout[:200]}")
                print(f"[ESCROW_CLAIM] stderr: {result.stderr[:200]}")
                raise HTTPException(500, f"Failed to parse on-chain state: {json_err}")
        else:
            print(f"[ESCROW_CLAIM] ❌ get_deposit_history failed:")
        ...
        print(f"[ESCROW_CLAIM] ❌ Error fetching on-chain state: {e}")
        raise
    ...
    try:
        ...
        print(f"[ESCROW_CLAIM] ✅ Added to on-chain tree at index {leaf_index}")
    ...
    if actual_leaf_index != leaf_index:
        print(f"[ESCROW_CLAIM] ⚠️  WARNING: Expected leaf_index {leaf_index} but local tree has {actual_leaf_index}")
    ...
    print(f"[ESCROW_CLAIM] ✅ Local tree updated, now has {len(tree.leaves)} leaves")
